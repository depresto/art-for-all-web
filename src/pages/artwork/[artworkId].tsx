...
const ArtworkPage: React.FC = () => {
  ...
  const { loadingArtwork, artwork, refetchArtwork } = useArtwork(parseInt(artworkId), currentMemberId || 0)
  ...
  return (
    <MainLayout title="作品賞析" activeKey="home">
      <Skeleton paragraph={{ rows: 10 }} loading={loadingArtwork}>
        {artwork && <ArtworkSection artwork={artwork} onRefetch={() => refetchArtwork()} />}
      </Skeleton>
    </MainLayout>
  )
}
