...
const AudioBookHomePage: React.FC = () => {
  ...
  const { loadingAudioBookCategories, audioBookCategories } = useAudioBooksWithCategories({
    ...
    <MainLayout title="有聲書" activeKey="audioBookHome">
      ...
      <Skeleton loading={loadingTags}>
        <div className="mb-3 px-1 d-flex">
          <AudioBookTypeTag className="mx-1" index={0} onClick={() => setActiveTagName(undefined)}>
            全部
          </AudioBookTypeTag>
            ...
            if (index % 2 === 0) {
              return (
                <div className="mb-3" key={index}>
                  <TitleBlock className="mb-3">{audioBookCategory.name}</TitleBlock>
                  <StyledTextDiv className="px-3">{audioBookCategory.description}</StyledTextDiv>
                  <div className="px-2 mt-2">
                    <AudioBookListSlider audioBooks={audioBookCategory.audioBooks} categoryId={audioBookCategory.id} />
                  </div>
                </div>
              )
            } else {
                  ...
                  <div className="px-1 mt-2">
                    <Row>
                      <Col span={12} className="px-1">
                        <AudioBookListSmallSlider audioBooks={audioBookPart1} categoryId={audioBookCategory.id} />
                      </Col>

                      <Col span={12} className="px-1">
                        <AudioBookListSmallSlider audioBooks={audioBookPart2} categoryId={audioBookCategory.id} />
                      </Col>
                    </Row>
                  </div>
                </div>
